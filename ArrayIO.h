...
template<class T> std::istream &operator >> (std::istream &s, Array<T> &x);
template<class T> bool read(std::istream &s, Array<T> &x,
...
template<class T> bool readArrayBlock(std::istream &s, bool &trans,
				      IPosition &p,
				      std::vector<T> &x,
				      const IPosition *ip=0, bool it=false);
