...
void testMeanComplex()
{
  ...
  BOOST_CHECK_CLOSE(m, res1/float(a.size()), 1e-6);
  BOOST_CHECK_CLOSE(rms(a), std::sqrt(res2/float(a.size())), 1e-6);
  ...
  BOOST_CHECK_CLOSE_FRACTION(v, res1/float(a.size()-1), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pv, res1/float(a.size()), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(variance(a, m), v, 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pvariance(a, m, 1), v, 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pvariance(a, m, 0), pv, 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pvariance(a, m), pv, 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(av, res2/float(a.size()), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(avdev(a, m), av, 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(stddev(a), std::sqrt(v), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pstddev(a), std::sqrt(pv), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(stddev(a, m), std::sqrt(v), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pstddev(a, m), std::sqrt(pv), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pstddev(a, m, 0), std::sqrt(pv), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(pstddev(a, m, 1), std::sqrt(v), 1e-5);
  IPosition start(3,0,1,2);
  IPosition end(3,2,3,4);
...
{\
  ...
  Array<U> z = FUNC(a); \
  for (size_t i=0; i<a.size(); ++i) {\
    BOOST_CHECK_CLOSE_FRACTION(ep[i], z.data()[i], 1e-13); \
    BOOST_CHECK(near(ep[i], z.data()[i], 1e-13)); \
  } \
