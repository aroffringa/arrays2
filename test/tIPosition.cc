...
BOOST_AUTO_TEST_CASE( common_operations )
{
  ...
  casacore::AipsIO io("tIPosition_tmp.data", casacore::ByteIO::New);
  io << ip3;
  io.close();
  io.open("tIPosition_tmp.data", casacore::ByteIO::Old);
  IPosition ip4;
  io >> ip4;
  BOOST_CHECK (ip4 == ip3);
  io.close();
  io.open("tIPosition_tmp.data", casacore::ByteIO::Delete);
  io.close();
}
...
BOOST_AUTO_TEST_CASE( move_exhaustively )
{
  ...
  casacore::AipsIO io("tIPosition_tmp.data", casacore::ByteIO::New);   // AipsIO << and >>
  IPosition iptmp1;
  io << ip1 << ip2 << iptmp1;
  io.close();
  io.open("tIPosition_tmp.data", casacore::ByteIO::Old);
  IPosition iptmp2;
  io >> iptmp2;
  BOOST_CHECK (ip1 == iptmp2);
  io >> iptmp2;
  BOOST_CHECK (ip2 == iptmp2);
  io >> iptmp2;
  BOOST_CHECK (iptmp1 == iptmp2);
  io.close();
  io.open("tIPosition_tmp.data", casacore::ByteIO::Delete);
  io.close();
