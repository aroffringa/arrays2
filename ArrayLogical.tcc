...
{
  LogicalArray result(array.shape());
  arrayContTransform (array, result, std::isnan);
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (array, result, std::isinf);
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (array, result, std::isfinite);
  return result;
}
...
LogicalArray near (const Array<T> &l, const Array<T>& r, double tol)
{
  ...
  LogicalArray result(l.shape());
  arrayContTransform (l, r, result, [tol](T left, T right){ return near(left, right, tol); });
  return result;
}
              				    ...
              				    double tol)
{
  ...
  LogicalArray result(l.shape());
  arrayContTransform (l, r, result, [tol](T left, T right){ return nearAbs(left, right, tol); });
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (array, val, result, [tol](T left, T right){ return near(left, right, tol); });
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (val, array, result, [tol](T left, T right){ return near(left, right, tol); });
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (array, val, result, [tol](T left, T right){ return nearAbs(left, right, tol); });
  return result;
}
...
{
  LogicalArray result(array.shape());
  arrayContTransform (val, array, result, [tol](T left, T right){ return nearAbs(left, right, tol); });
  return result;
}
...
template<class T> bool allNear (const Array<T> &l, const Array<T> &r,
				double tol)
{
  return arrayCompareAll (l, r, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool allNear (const Array<T> &array, const T &val, double tol)
{
  return arrayCompareAll (array, val, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool allNear (const T &val, const Array<T> &array, double tol)
{
  return arrayCompareAll (val, array, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool allNearAbs (const Array<T> &l, const Array<T> &r,
				   double tol)
{
  return arrayCompareAll (l, r, [tol](T left, T right){ return nearAbs(left, right, tol); });
}

template<class T> bool allNearAbs (const Array<T> &array, const T &val,
				   double tol)
{
  return arrayCompareAll (array, val, [tol](T left, T right){ return nearAbs(left, right, tol); });
}

template<class T> bool allNearAbs (const T &val, const Array<T> &array,
				   double tol)
{
  return arrayCompareAll (val, array, [tol](T left, T right){ return nearAbs(left, right, tol); });
}
...
template<class T> bool anyNear (const Array<T> &l, const Array<T> &r,
				double tol)
{
  return arrayCompareAny (l, r, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool anyNear (const Array<T> &array, const T &val, double tol)
{
  return arrayCompareAny (array, val, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool anyNear (const T &val, const Array<T> &array, double tol)
{
  return arrayCompareAny (val, array, [tol](T left, T right){ return near(left, right, tol); });
}

template<class T> bool anyNearAbs (const Array<T> &l, const Array<T> &r,
				   double tol)
{
  return arrayCompareAny (l, r, [tol](T left, T right){ return nearAbs(left, right, tol); });
}

template<class T> bool anyNearAbs (const Array<T> &array, const T &val,
				   double tol)
{
  return arrayCompareAny (array, val, [tol](T left, T right){ return nearAbs(left, right, tol); });
}

template<class T> bool anyNearAbs (const T &val, const Array<T> &array,
				   double tol)
{
  return arrayCompareAny (val, array, [tol](T left, T right){ return nearAbs(left, right, tol); });
}
...
template<class T> Array<unsigned> partialNTrue (const Array<T>& array,
					    const IPosition& collapseAxes)
{
  const IPosition& shape = array.shape();
  unsigned ndim = shape.nelements();
  if (ndim == 0) {
    return Array<unsigned>();
  }
  IPosition resShape, incr;
  int nelemCont = 0;
  unsigned stax = partialFuncHelper (nelemCont, resShape, incr, shape,
				 collapseAxes);
  Array<unsigned> result (resShape);
  result = 0;
  bool deleteData, deleteRes;
  const T* arrData = array.getStorage (deleteData);
  const T* data = arrData;
  unsigned* resData = result.getStorage (deleteRes);
  unsigned* res = resData;
...
template<class T> Array<unsigned> partialNFalse (const Array<T>& array,
					     const IPosition& collapseAxes)
{
  Array<unsigned> result = partialNTrue (array, collapseAxes);
  unsigned nr = result.nelements();
  if (nr > 0) {
    unsigned factor = array.nelements() / nr;
    bool deleteRes;
    unsigned* res = result.getStorage (deleteRes);
